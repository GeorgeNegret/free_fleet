...
class Nav2RobotAdapter(RobotAdapter):
    ...
    def navigate(self, destination: rmf_easy.Destination, execution: rmf_easy.CommandExecution):
        ...
        if current_pose is None:
            # If current pose is not available, fallback to provided yaw
            self.node.get_logger().warn(
                f'Unable to get current pose for {self.name}. Using provided yaw value.'
            )
            desired_yaw = destination.position[2]
        else:
            desired_yaw = current_pose[2]
        
        # Stocăm ținta cu yaw-ul fix
        self._current_goal_pose = (destination.position[0], destination.position[1], desired_yaw)

        self.node.get_logger().info(
            f'Commanding [{self.name}] to navigate to position '
            f'({destination.position[0]}, {destination.position[1]}) on map [{destination.map}] '
            f'while maintaining orientation {desired_yaw}'
        )
